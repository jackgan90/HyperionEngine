...
def run(*args, fail=False):
    ...
    run.count += 1
    if fail:
        expected = fail if isinstance(fail, str) else "Missing plugin"
...
assert restored["rhi_backend"] == "d3d12"
...
initial.write_text(json.dumps(base), encoding="utf-8")
run("--config", initial, fail="RHI backend is not registered: metal")
run("--config", initial, "--backend", "d3d12")
