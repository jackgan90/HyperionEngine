...
def import_asset(viewer, source, output=None, *, mounts=None):
    ...
    arguments = [str(tool)] + (["--mounts", str(mounts)] if mounts else [])
...
def set_initial_view_from_camera(viewer, scene, output=None, *, mounts=None):
    ...
    return import_asset(viewer, source, output, mounts=mounts)
