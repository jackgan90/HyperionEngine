"""End-to-end Viewer CLI: external glTF, embedded GLB, model UI and reported errors."""
import pathlib
import subprocess
import sys
...
for name, show_ui in [("Showcase.gltf", True), ("Showcase.glb", False), ("Missing.gltf", False)]:
    capture = root / "out/captures" / ("model-viewer-" + name + ".png")
    args = [str(viewer), "--model", str(root / "out/fixtures" / name),
    ...
    if name == "Missing.gltf":
        assert result.returncode != 0 and "Missing.bin" in log, log
    else:
...
result = subprocess.run(
    [str(viewer), "--model", str(root / "out/fixtures/Showcase.glb"),
...
print("External glTF, embedded GLB, model UI, missing dependency and failed-save shutdown acceptance passed")
