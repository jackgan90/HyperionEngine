...
template<> const FRecordDescriptor& RecordType<FLegacySceneManifest>()
{
	static const auto Type = []
	{
		auto Result = MakeRecord<FLegacySceneManifest>(
		    "hyperion.scene",
		    {Member("assets", &FLegacySceneManifest::Assets, {true}),
		     Member("instances", &FLegacySceneManifest::Instances, {true}), Member("eye", &FLegacySceneManifest::Eye),
		     Member("target", &FLegacySceneManifest::Target), Member("near", &FLegacySceneManifest::Near),
		     Member("far", &FLegacySceneManifest::Far)},
		    3, ValidateLegacySceneManifest);
		Result.Migrations.emplace(1,
		                          [](FArchiveNode::FObject&)
		                          {
		                          });
