...
using FSceneHandleMap = std::unordered_map<FSceneHandle, FSceneHandle, FSceneHandleHash>;

// Selection order is explicit editor state; the last remaining object owns the gizmo.
class FSceneSelection
{
public:
	FSceneSelection() = default;
	FSceneSelection(const FSceneSelection&) = default;
	FSceneSelection(FSceneSelection&&) noexcept = default;
	FSceneSelection& operator=(FSceneSelection&&) noexcept = default;

	FSceneSelection& operator=(const FSceneSelection& InOther)
	{
		FSceneSelection Updated(InOther);
		Swap(Updated);
		return *this;
	}

	FSceneSelection(std::optional<FSceneHandle> InHandle)
	{
		*this = InHandle;
	}

	FSceneSelection& operator=(std::optional<FSceneHandle> InHandle)
	{
		FSceneSelection Updated;
		if (InHandle)
		{
	...
	bool operator==(const FSceneSelection& InOther) const
	{
	...
	{
		Remap(FSceneHandleMap{{InBefore, InAfter}});
	}

	void Remap(const FSceneHandleMap& InMapping)
	{
		FSceneSelection Updated;
...
private:
	...
	std::unordered_set<FSceneHandle, FSceneHandleHash> Membership;
};
} // namespace Hyperion
