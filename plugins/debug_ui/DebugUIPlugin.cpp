...
FDebugUiPlugin::FDebugUiPlugin(FRhiDevice& InDevice, FShaderCompiler& InCompiler, FTaskSystem& InTasks, FImage InFont)
...
void FDebugUiPlugin::Start()
{
	...
	P.Tasks.Wait(P.Tasks.Dispatch(
	    ...
	    [&]
	    {
		    Desc.Vertex = P.Compiler.Compile("Gui.hlsl", "VSMain", EShaderStage::Vertex, EShaderFormat::Dxil);
		    Desc.Pixel = P.Compiler.Compile("Gui.hlsl", "PSMain", EShaderStage::Pixel, EShaderFormat::Dxil);
	    }));
