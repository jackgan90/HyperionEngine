...
组件以对象世界原点为中心，X/Y/Z 分别使用红、绿、蓝色，悬停或拖拽的手柄显示黄色。组件在场景图像上叠加显示，不受模型遮挡，大小随界面缩放保持稳定；相机预览模式下停止操纵。此版本不提供物体高亮或点击场景物体选择。
...
- Details 支持反射编辑、保存与撤销重做，并提供从编辑器视角创建相机的入口；可通过 FGuiPanelEvent 订阅绘制扩展面板；内置文档面板仍属于同一个 Editor 插件。PRS 操纵支持 Outliner 中选定的对象；尚无 Play、通用资产选择器或通用对象创建删除工具。模型替换和新实例创建使用 Runtime 或导入工具。
