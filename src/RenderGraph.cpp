...
{
	if (Passes.empty() || Passes.size() > 15)
	{
		throw std::runtime_error("Graph requires 1 to 15 color passes");
	}
...
FImage ExecuteGraph(const FRenderGraph& InGraph, FTaskSystem& InTasks, FRhiDevice& InDevice, FSize InSize, bool InVsync,
                    bool InCapture)
{
	auto Commands = InGraph.Compile(); // Fail before beginFrame if the graph is invalid.
	InTasks.Wait(InTasks.Dispatch({EDomain::Rhi, 0},
	                              [&]
	                              {
		                              InDevice.BeginFrame(InSize);
	                              }));
	std::vector<FRecordedList> Lists(Commands.size());
	std::vector<FTaskHandle> Recordings;
	for (std::size_t I = 0; I < Commands.size(); ++I)
	{
		Recordings.push_back(InTasks.Dispatch({EDomain::Rhi, static_cast<std::uint32_t>(I % InTasks.RhiThreadCount())},
		                                      [&, I]
		                                      {
			                                      Lists[I] =
			                                          InDevice.Record(static_cast<std::uint32_t>(I), Commands[I]);
		                                      }));
	}
	...
	InTasks.Wait(InTasks.Dispatch({EDomain::Rhi, 0},
	                              [&]
	                              {
		                              Image = InDevice.EndFrame(Lists, InVsync, InCapture);
	                              }));
	return Image;
}
} // namespace Hyperion
