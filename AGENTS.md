...
- A single function should normally not exceed 100 lines. Split longer functions at logical boundaries unless the logic is so tightly coupled that meaningful decomposition is difficult; document that exception near the function. Follow the counting and review guidance in `docs/CodingStyle.md`.
...
- Scene/Animation are independent data-oriented runtime modules when introduced; do not place them in Renderer or make them depend on RHI. Keep shared utilities in focused Core subdirectories and module-specific helpers private.
